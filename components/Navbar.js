...
function Navbar({
  ...
  return (
    <header className={`text-gray-600 body-font ${!sidebar && "overflow-x-hidden"}`}>
      ...
      <div
        ref={ref}
        className={` w-60 h-[100vh] sideCart overflow-y-scroll fixed  z-10 top-0 p-8  bg-pink-100  transition-all  ${sidebar ? "right-0" : "-right-64"} `}
      >
        {" "}
        ...
        <div className="flex">
          <Link href="/checkout">
            <a>
              <button disabled={Object.keys(cart).length == 0} className="flex mr-2 disabled:bg-pink-200  mt-0 text-white bg-pink-500 border-0 py-1 px-1 focus:outline-none hover:bg-pink-600 rounded text-sm">
          ...
          <button
            ...
            onClick={clearCart}
            className="flex  mt-0 text-white disabled:bg-pink-200 bg-pink-500 border-0 py-1 px-1 focus:outline-none hover:bg-pink-600 rounded text-sm"
          >
            clear Cart
          </button>
        </div>
